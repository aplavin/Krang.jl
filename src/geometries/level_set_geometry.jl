...
@inline function line_intersection(
    ...
    origin = Krang.boyer_lindquist_to_quasi_cartesian_kerr_schild_fast_light(
        pixel.metric,
        origin_bl[1:3]...,
    )
    line_point_2_bl = Krang.emission_coordinates_fast_light(pixel, τf)
    line_point_2 = Krang.boyer_lindquist_to_quasi_cartesian_kerr_schild_fast_light(
        pixel.metric,
        line_point_2_bl[1:3]...,
    )
    ...
    if didintersect
        t = Krang.Roots.find_zero(
            (x) ->
                x == zero(T) ? geo_origin :
                geometry(
                    Krang.boyer_lindquist_to_quasi_cartesian_kerr_schild_fast_light(
                        pixel.metric,
                        Krang.emission_coordinates_fast_light(pixel, x)[1:3]...,
                    )...,
                ),
            (τi, τf),
        )
